...
class AuthService:
    ...
    async def login_user(self, login_in: LoginRequest) -> UserResponse:
        ...
        try:
            # 3. Consume the verified OTP so it can't be reused
            await self.otp_repo.consume_verified_otp(login_in.email, "login")
            
            # 4. Auto-verify user status upon successful login if not already verified
    ...
    async def reset_password(self, reset_in: ForgotPasswordRequest) -> None:
        ...
        try:
            # 4. Update password and consume OTP
            hashed_pw = hash_password(reset_in.new_password)
            await self.user_repo.update_user_password(user, hashed_pw)
            await self.otp_repo.consume_verified_otp(reset_in.email, "forgot_password")
            await self.user_repo.db.commit()
        except Exception as e:
            await self.user_repo.db.rollback()
            raise e
